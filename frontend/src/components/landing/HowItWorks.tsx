        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-6 relative">
          ...
          {steps.map((step, i) => (
            <div key={step.number} className="relative">
              {/* Step card */}
              <div className="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300 group h-full flex flex-col">
                ...
                <p className="text-gray-600 leading-relaxed mb-5 text-sm flex-1">{step.description}</p>

                {/* Code block */}
                <div className="bg-blue-50/70 border border-blue-200 rounded-xl p-4 overflow-x-auto mt-auto">
                  <pre className="text-xs sm:text-sm text-gray-700 font-mono leading-relaxed whitespace-pre-wrap">
                    <code>{step.code}</code>
                  </pre>
                </div>
              </div>
